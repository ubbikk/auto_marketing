...
def build_generator_prompt_explanatory(
    source,
    persona: dict,
    ctx: CreativityContext,
    num_variants: int,
    ...
    """Build generator prompt for explanatory mode (no company context).

    Args:
        source: SourceContent with title, source, summary, key_insights.
        persona: Persona configuration dict.
        ctx: Creativity context with examples, style, tone, etc.
        num_variants: Number of variants to generate.
        num_generators: How many generators run in parallel.
    ...
    # Build conditional sections (same as marketing mode)
    few_shot_section = ""
    if ctx.few_shot_examples:
        few_shot_section = (
            "\nFEW-SHOT EXAMPLES (study these, don't copy verbatim):\n"
    ...
    if hasattr(source, "key_insights") and source.key_insights:
        key_insights_section = (
            "\nKEY INSIGHTS FROM SOURCE:\n"
            + "\n".join(f"- {insight}" for insight in source.key_insights)
            + "\n"
        )

    return render(
        "generator_explanatory",
        num_generators=num_generators,
        persona_name=persona_name,
        voice_traits="\n".join(f"- {t}" for t in voice_traits),
        relationship=relationship,
