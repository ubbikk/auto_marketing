...
def _render_content_slide(slide, slide_num: int, total: int) -> str:
    """Render a single explanatory content slide based on its type."""
    # Alternate glow positions for visual variety
    glow_variants = [
        '<div class="glow glow--cyan" style="width:500px;height:500px;top:50%;left:-140px;opacity:0.18;transform:translateY(-50%);"></div>',
        '<div class="glow glow--accent" style="width:450px;height:450px;bottom:-100px;right:-80px;opacity:0.2;"></div>',
        '<div class="glow glow--purple" style="width:400px;height:400px;top:-80px;right:-100px;opacity:0.2;"></div>',
        '<div class="glow glow--cyan" style="width:500px;height:500px;top:-100px;left:50%;opacity:0.2;transform:translateX(-50%);"></div>',
    ]
    glow = glow_variants[(slide_num - 2) % len(glow_variants)]

    # Build content based on slide type
    if slide.type == "bullet":
        items_html = "\n".join(f"<li>{_esc(b)}</li>" for b in slide.bullets)
        content_html = f"""\
      <h2 class="heading heading--lg gradient-text">{_esc(slide.heading)}</h2>
      <hr class="divider" style="width:120px;">
      <ul class="bullet-list">
        {items_html}
      </ul>"""

    elif slide.type == "numbered":
        items_html = ""
        ...
        content_html = f"""\
      <h2 class="heading heading--md gradient-text">{_esc(slide.heading)}</h2>
      <hr class="divider" style="width:120px;">
      <ol class="number-list">
        {items_html}
      </ol>"""
      ...
      <h2 class="heading heading--md gradient-text">{_esc(slide.heading)}</h2>
      <hr class="divider" style="width:120px;">
      <div class="glass-card" style="display:flex; gap:20px; align-items:flex-start; margin-top: 16px;">
        <span class="quote-mark">\u201c</span>
        <div>
          <p style="font-size:26px; line-height:1.55; color:rgba(232,234,246,0.75); font-style:italic;">{_esc(slide.quote_text)}</p>
          <p style="font-size:18px; color:var(--primary-400); margin-top:16px; font-weight:600;">{_esc(slide.quote_attribution)}</p>
        </div>
      </div>"""
      ...
      <div style="display:flex; gap:32px;">
        ...
        content_html = f'<h2 class="heading heading--md gradient-text">{_esc(slide.heading)}</h2>'

    return f"""\
<!-- SLIDE {slide_num} — {slide.type.upper()} -->
<div class="slide" id="slide-{slide_num}">
  ...
  <span class="slide-counter">{slide_num} / {total}</span>
</div>
"""


def build_html_explanatory(content: ExplanatoryCarouselContent) -> str:
    """Build a standalone HTML document for explanatory carousel (no branding).

    Args:
        content: Structured explanatory carousel content with flexible slides.
    ...
    total = 2 + len(content.slides)  # cover + content slides + closing

    # Build all content slides
    content_slides_html = ""
    for i, slide in enumerate(content.slides):
        slide_num = i + 2  # Cover is slide 1
        content_slides_html += _render_content_slide(slide, slide_num, total)

    # Build source attribution for closing slide
    source_line = ""
    if closing.source_title:
        source_line = _esc(closing.source_title)
        if closing.source_author:
            source_line += f" — {_esc(closing.source_author)}"
...
<!-- SLIDE {total} — CLOSING -->
<div class="slide" id="slide-{total}">
  <div class="accent-stripe"></div>
  <div class="glow glow--cyan" style="width:700px;height:700px;top:50%;left:50%;opacity:0.25;transform:translate(-50%,-50%);"></div>
  <div class="glow glow--accent" style="width:350px;height:350px;bottom:-60px;right:-60px;opacity:0.15;"></div>
  <div class="slide-inner" style="justify-content:center; align-items:center; text-align:center; gap:48px;">
    <h2 class="heading heading--lg gradient-text--warm" style="max-width:800px;">{_esc(closing.takeaway)}</h2>
    <hr class="divider" style="width:200px;">
    <p class="subtitle" style="max-width:640px;">{source_line}</p>
  </div>
